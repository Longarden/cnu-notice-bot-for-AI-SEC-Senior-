...
**구독 방법: https://t.me/cnu_alarmbot 에서 `/start` 보내면 끝.**

바로 "구독 완료" 답장이 오고, 다음 발송부터 그 대화방으로 알림이 간다.
그만 받고 싶으면 `/stop`. 관리자가 설정을 바꿀 필요는 없다.

채널로도 뿌리고 싶으면 채널을 만들어 `TELEGRAM_CHAT_ID`에 `@채널명`을 넣으면 된다(선택).
...
| 워크플로우 | 주기 | 하는 일 |
|-----------|------|---------|
| `notify.yml` | 매일 UTC 00:00 (한국 오전 9시) | 전체 수집 + 구독자 전원에게 발송 |
| `greet.yml` | 3시간마다 | `/start`·`/stop` 처리 + 명부 갱신 (발송 없음) |

구독 처리를 따로 뗀 이유: 텔레그램은 **미확인 update를 24시간만 보관**한다.
하루 한 번짜리 워크플로우 하나에만 의존하면 그 사이 `/start` 한 사람을 놓친다.
반대로 공지 다이제스트를 3시간마다 보내면 같은 공지가 하루 8번 오게 되므로,
자주 도는 쪽은 발송을 하지 않는다.

`/start` 하고 최대 3시간 안에 "구독 완료" 답장이 온다. 답장을 받은 다음 날 아침부터 알림이 온다.
...
**기본은 아무 설정도 필요 없다.** 봇에게 `/start`를 보낸 사람이 자동으로 발송
대상이 된다. 친구에게는 봇 링크만 보내면 된다.

`TELEGRAM_CHAT_ID`는 **추가** 대상을 지정할 때만 쓴다. 비워둬도 동작한다.

| 넣는 값 | 의미 |
|---------|------|
| (비움) | `/start` 한 구독자에게만 발송 |
| `123456789` | 그 개인 chat_id에도 항상 발송 (`@userinfobot`으로 확인 가능) |
| `@cnu_alarm` | 채널에도 발송 |
| `@cnu_alarm,123456789` | 둘 다 |

**채널을 쓰고 싶다면** (선택):

1. 텔레그램 → 연필 아이콘 → **새 채널** → 이름 입력
2. **공개 채널**로 두고 링크 주소를 정한다 (예: `cnu_alarm` → `https://t.me/cnu_alarm`)
3. 채널 → 편집 → **관리자** → 관리자 추가 → `@cnu_alarmbot` 검색해서 추가
4. 관리자 권한 중 **메시지 게시**가 켜져 있어야 한다 (꺼져 있으면 발송이 실패한다)
5. `TELEGRAM_CHAT_ID`에 `@cnu_alarm` 을 넣는다

채널을 지정해두면 채널 발송 실패는 워크플로우를 빨간 X로 만든다(주 경로로 취급).
반면 개인 구독자 한 명이 봇을 차단한 것은 초록으로 넘어가고, 그 사람은 명부에서 자동 제외된다.
...
| `TELEGRAM_CHAT_ID` | ❌ | 추가 발송 대상. 없으면 `/start` 구독자에게만 발송 |

**Variables** 탭 (New repository variable) — 비밀이 아니라 그냥 링크라서 variable로 둔다:

| Name | 필수 | Value |
|------|------|-------|
| `TELEGRAM_CHANNEL_LINK` | ❌ | 채널을 만들었다면 `https://t.me/cnu_alarm`. `/start` 답장에 채널 안내가 함께 붙는다 |

즉 **실제로 반드시 있어야 하는 건 `TELEGRAM_BOT_TOKEN` 하나뿐이다.**
...
- **`/start`를 눌렀는데 답장이 없다**: `greet.yml`이 3시간마다 돌므로 최대 3시간 기다린다.
  급하면 Actions 탭 → `CNU Notice Greeter` → `Run workflow`로 즉시 실행할 수 있다.
  **24시간 넘게 방치하면 텔레그램이 그 `/start`를 지우므로 다시 눌러야 한다.**
- **답장은 받았는데 공지가 안 온다**: 다음 아침 9시 발송을 기다린다. `py roster.py`로
  본인 chat_id가 명부에 있는지 확인할 수 있다.
...
- **갑자기 알림이 끊겼다**: 봇을 차단했거나 `/stop`을 보낸 경우 명부에서 빠진다.
  `/start`를 다시 보내면 복구된다.
